// ============================================================
//  DoctorHeader.jsx – Reusable Doctor Top Header Component
// ============================================================

import React, { useEffect, useState } from 'react';
import '../../style/doctor/DoctorLayout.css';

export default function DoctorHeader({
  docName = 'Dr. Sharma',
  docSpecialty = 'Cardiology Department',
  roomNumber = 'Room 402B',
  onOpenSettings,
  onOpenProfile,
}) {
  const [searchQuery, setSearchQuery] = useState('');
  const [notificationsOpen, setNotificationsOpen] = useState(false);
  const [name, setName] = useState("");

  const id = localStorage.getItem("doctorId");

  useEffect(() => {
    fetch(`http://localhost:8000/doctor/${id}`)
    .then((res) => res.json())
    .then((data) => {
      setName(data.name);
    })
  },[])

  return (
    <header className="bg-white border-b border-slate-200 px-8 py-5 flex items-center justify-between sticky top-0 z-40 shadow-sm">
      <div>
        <h2 className="text-2xl font-black text-slate-900">Welcome back, {docName}</h2>
        <p className="text-xs font-semibold text-slate-400 mt-0.5">
          {docSpecialty} • {roomNumber} • Live OPD Session
        </p>
      </div>
      <div className="flex items-center gap-4 relative">
        <div className="relative hidden lg:block">
          <input
            type="text"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search patient, PID or token..."
            className="w-64 pl-10 pr-4 py-2.5 bg-slate-100 border border-transparent rounded-2xl text-xs font-medium text-slate-800 focus:bg-white focus:border-slate-300 focus:outline-none transition-all"
          />
          <span className="material-symbols-outlined absolute left-3 top-2.5 text-slate-400 text-lg">
            search
          </span>
        </div>

        <button
          onClick={() => setNotificationsOpen(!notificationsOpen)}
          className="p-2.5 bg-slate-100 hover:bg-slate-200 rounded-2xl text-slate-600 relative transition-colors"
        >
          <span className="material-symbols-outlined text-xl">notifications</span>
          <span className="absolute top-2 right-2 w-2 h-2 bg-red-500 rounded-full border border-white"></span>
        </button>

        {notificationsOpen && (
          <div className="absolute top-14 right-24 w-80 bg-white rounded-2xl shadow-2xl border border-slate-200 p-4 z-[100] animate-fadeIn space-y-3">
            <div className="flex items-center justify-between pb-2 border-b border-slate-100">
              <span className="font-bold text-sm text-slate-900">Clinical Alerts</span>
              <span className="text-[10px] bg-red-100 text-red-700 px-2 py-0.5 rounded-full font-extrabold">
                1 Urgent
              </span>
            </div>
            <div className="space-y-2 text-xs">
              <div className="p-2.5 bg-red-50 rounded-xl">
                <p className="font-bold text-slate-900">Critical Lab Result</p>
                <p className="text-slate-500 mt-0.5">Patient: Manish Gupta (Troponin-I 0.45 ng/mL)</p>
              </div>
              <div className="p-2.5 bg-blue-50 rounded-xl">
                <p className="font-bold text-slate-900">X-Ray PA View Uploaded</p>
                <p className="text-slate-500 mt-0.5">Patient: Reema Verma (Chest Scan)</p>
              </div>
            </div>
          </div>
        )}

        <button
          onClick={onOpenSettings}
          className="p-2.5 bg-slate-100 hover:bg-slate-200 rounded-2xl text-slate-600 transition-colors"
        >
          <span className="material-symbols-outlined text-xl">settings</span>
        </button>

        <div
          onClick={onOpenProfile}
          className="flex items-center gap-2 pl-2 cursor-pointer hover:opacity-90 transition-opacity"
        >
          <div className="w-10 h-10 rounded-full bg-blue-600 text-white font-bold flex items-center justify-center border-2 border-blue-500 shadow-sm text-sm">
            {name.split(" ")[0]?.charAt(0) || ""}
            {name.split(" ")[1]?.charAt(0) || ""}
          </div>
        </div>
      </div>
    </header>
  );
}
